...
int computeUpperLowerHull(vector<Point> &P, int startPos, int endPos, vector<Point> &Hupper, vector<Point> &Hlower )
{
    ...
    while (++i <= maxMin)
    {
        ...
        if (relativePosition( P.at(minMin), P.at(maxMin), P.at(i))== ON && i < maxMin) // the lower line joins P[minMin]  with P[maxMin] which divides the upper and lower convex hull
            continue;  
		...
		{
			if (relativePosition( P.at(minMin), P.at(maxMin), P.at(i))== BELOW && i <= maxMin)
			{
				while (topUpper > 0)         // there are at least 2 points on the stack
				{
                    if (relativePosition(  Hupper.at(topUpper-1), Hupper.at(topUpper), P.at(i))== ABOVE) // test if  P[i] is left of the line at the stack top
			...
			else
			{
               while (topLower > 0)         // there are at least 2 points on the stack
                 {
					 if (relativePosition(  Hlower.at(topLower-1), Hlower.at(topLower), P.at(i))== BELOW) // test if  P[i] is left of the line at the stack top
                         break;         // P[i] is a new hull  vertex
                      else
			            {
				         Hlower.pop_back();
...
void upperTangentComputationParallel(vector<Point> &H1upper,vector<Point> &H2upper,vector<Point> &H3upper,vector<Point> &H4upper,int N)
{
	...
	tangentBetweenTwoUpperHulls(H1upper.size(),H1upper,H2upper.size(),H2upper,H1RightTangentPoint,H2LeftTangentPoint);
	tangentBetweenTwoUpperHulls(H1upper.size(),H1upper,H3upper.size(),H3upper,H1RightTangentPoint,H3LeftTangentPoint);
	tangentBetweenTwoUpperHulls(H1upper.size(),H1upper,H4upper.size(),H4upper,H1RightTangentPoint,H4LeftTangentPoint);
	tangentBetweenTwoUpperHulls(H2upper.size(),H2upper,H3upper.size(),H3upper,H2RightTangentPoint,H3LeftTangentPoint);
	tangentBetweenTwoUpperHulls(H2upper.size(),H2upper,H4upper.size(),H4upper,H2RightTangentPoint,H4LeftTangentPoint);
	tangentBetweenTwoUpperHulls(H3upper.size(),H3upper,H4upper.size(),H4upper,H3RightTangentPoint,H4LeftTangentPoint);
	...
	//Compute r[i]s
	rUpper.push_back(H1RightTangentPoint[H1RightTangentPoint.size()-1]);
	rUpper.push_back(H2RightTangentPoint[H2RightTangentPoint.size()-1]);
